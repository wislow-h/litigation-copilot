...
## Solar Pro 3 — 응답 크기·지연 시간 실측 (324p 스캔본, 2026-06-12~13)

324페이지 스캔본(업무상배임 1심 기록)을 OCR 후 24K자 단위 14청크로 나눠 `solar-pro3` 구조화 출력(strict json_schema) 추출을 돌리며 측정했다.

### 발견 1 — 지연 시간은 **입력 크기가 아니라 출력 토큰 수**에 비례
입력은 전 청크가 ~10–16K 토큰으로 거의 동일한데, 소요 시간은 4초~72초로 갈렸고 **출력 토큰 수에 정비례**했다(약 100–120 tok/s).
...
→ 이벤트가 밀집된 구간일수록 거대한 JSON을 생성해야 하고, 그게 그대로 지연으로 이어진다. **입력을 줄이기보다 청크를 잘게 나눠 청크당 출력을 줄이는 것**이 지연·실패를 동시에 낮춘다.

### 발견 2 — **동일 입력인데도 응답 시간이 21초~1,086초로 요동** (서빙 안정성)
글자 한 자 안 바꾼 동일 청크(23,813자)를 `temperature:0`으로 6회 반복 호출한 결과:

| 회차 | 1 | 2 | 3 | 4 | 5 | 6 |
|---|---|---|---|---|---|---|
| 결과 | ❌ 180s 타임아웃 | ✅ 23.0s | ❌ 1086.6s | ✅ 21.0s | ❌ 180s 타임아웃 | ❌ 979.1s |

**최소 21s · 최대 1,086s · 편차 51.7배 · 6회 중 4회 멈춤(hang).** 성공 시 출력 토큰은 2,605–2,789로 일정 → 모델이 하는 "작업"은 같은데 **서빙 지연만 요동**친다. 즉 입력/내용 문제가 아닌 **API 서빙단의 latency 불안정성**이다. 멈춘 호출은 클라이언트 180s 타임아웃을 넘겨 ~1,000s까지 매달리기도 해(연결이 끊기지 않고 응답만 지연), 클라이언트 측 타임아웃만으로는 깔끔히 회수되지 않았다.

### 시사점 (제품/운영 관점)
- 큰 구조화 출력을 요구하는 호출일수록 멈춤 위험이 급증한다. 가벼운 청크는 빠르고 안정적이었다.
- 완화책: ① **청크 축소**로 청크당 출력량 억제, ② **타임아웃+재시도**(일시적 멈춤은 재시도하면 대개 즉시 성공), ③ 출력 상한(`max_tokens`)으로 폭주 방지. 근본 안정성은 제공사 서빙단 개선 영역.
- 측정 스크립트는 일회성으로 작성·실행했고 리포지토리에는 남기지 않았다(본 문서의 수치가 요약본).
